...
# Gower mixed distance dissimilarity matrix
library(cluster)
...
dd[, actives] <- lapply(dd[, actives], function(x) {
  if (is.character(x)) {
    return(factor(x))
  } else {
    return(x)
...
# Identify columns that are categorical (have a small number of unique values)
categorical_columns <- c("meal", "country", "market_seg", "channel")

# Convert these columns to factors as well
dd[, categorical_columns] <- lapply(dd[, categorical_columns], factor)

# Define binary columns (from your check, 'canceled' is binary)
binary_columns <- c("canceled")

# Check the structure to verify the conversion
str(dd[, binary_columns])
